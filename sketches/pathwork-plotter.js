...
const sketch = ({ width, height, units, render }) => {
  ...
  const pointCount = filterdData.length;
  let points = Array.from(new Array(pointCount)).map((item, i) => {
    const margin = 2;
    return [
      random.range(margin, width - margin - filterdData[i]),
      random.range(margin, height - margin - filterdData[i])
    ];
  });
